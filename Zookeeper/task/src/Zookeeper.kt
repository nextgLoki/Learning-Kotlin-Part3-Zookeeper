...
fun main() {
    do {
        askCommand()
        val (cameraNumber, isNeedExit) = readCommand()
        if (!isNeedExit) {
            getCamera(cameraNumber).let(::showCamera)
        }
    } while (!isNeedExit)
    endMessage()
}

fun askCommand() = println("Please enter the number of the habitat you would like to view:")

fun readCommand(): Pair<Int, Boolean> {
    val command = readLine()!!
    val camera = command.toIntOrNull()
    return if (command == "exit") {
        Pair(0, true)
    } else if (camera != null && camera >= 0 && camera < animals.size) {
        Pair(camera, false)
    } else {
        readCommand()
    }
...
fun getCamera(camera: Int) = animals[camera]

fun showCamera(camera: String) = println(camera)

fun endMessage() = println("See you later!")